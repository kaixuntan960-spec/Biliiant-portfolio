...
const ensureSharedAudio = () => {
  if (typeof window !== "undefined") {
    ...
    if (!w.__portfolioSharedAudio) {
      ...
      w.__portfolioSharedAudio.volume = 0.52;
    }
...
const MusicPlayer = ({ playing = false, onToggle = () => {} }: MusicPlayerProps) => {
  ...
  useEffect(() => {
    ...
    // Sync UI with shared player on mount so route changes don't restart music.
    if (audioElRef.current) {
      syncPlaying(!audioElRef.current.paused);
      setIsAudioPlaying(!audioElRef.current.paused);
    }
    try {
  ...
  const handleNext = (e: React.MouseEvent) => {
    ...
    setTrackIndex((i) => (i + 1) % PLAYLIST.length);
    try {
      window.dispatchEvent(new CustomEvent("sfx", { detail: { kind: "yay" } }));
    } catch {
      // ignore
    }
  };
  ...
  const clampPos = (x: number, y: number) => {
    ...
    return {
      ...
      y: Math.max(margin, Math.min(window.innerHeight - PANEL_HEIGHT - margin, y)),
    };
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span className="inline-flex items-center justify-center flex-shrink-0" style={{ width: expanded ? "20px" : "52px", marginRight: expanded ? "8px" : 0 }}>
            {expanded ? (playing ? <Pause size={14} /> : <Play size={14} />) : null}
          </span>
        ...
        <button
          ...
          data-sfx="yay"
          title={lang === "en" ? "Next track" : "切换音乐"}
          aria-label={lang === "en" ? "Next track" : "切换音乐"}
        >
          <SkipForward size={12} />
        </button>
      </div>
