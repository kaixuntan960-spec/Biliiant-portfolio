import { Suspense, useEffect, useRef, useState, useCallback, lazy } from "react";
import { Heart, Sparkles } from "lucide-react";
import { useI18n } from "../i18n";
import { useSiteContent } from "../i18n";
import { useThemeMode } from "../theme";

const ClawMachineHero = lazy(() => import("./ClawMachineHero"));
  ...
  const toggleLike = () => {
    setLiked((v) => {
      ...
      setLikeCount((c) => {
        ...
        try { localStorage.setItem("portfolio_like_count", String(nextCount)); } catch {}
        return nextCount;
      });
      try { localStorage.setItem("portfolio_liked", next ? "1" : "0"); } catch {}
      return next;
    });
  };
    ...
    >
      ...
      <div ref={blobsRef} className="absolute inset-0 pointer-events-none overflow-hidden">
        <div
          ...
          style={{
            width: "50vw", height: "50vw", top: "-10%", left: "-5%",
            background: isDark
        ...
        <div
          ...
          style={{
            width: "40vw", height: "40vw", top: "40%", right: "-10%",
            background: isDark
        ...
        <div
          ...
          style={{
            width: "35vw", height: "35vw", bottom: "-5%", left: "30%",
            background: isDark
      ...
      {/* Claw machine — full section overlay, behind text */}
      ...
      >
        ...
        >
          <Heart size={18} style={{ color: liked ? "rgb(255,107,157)" : "var(--muted-foreground)", fill: liked ? "rgb(255,107,157)" : "transparent" }} />
        </button>
        <span
          style={{
            fontSize: "10px",
        ...
        >
          {lang === "en" ? `${likeCount} likes` : `${likeCount}人点赞`}
        </span>
      </div>

      {/* Main content: left text */}
      <div
        className={`relative z-10 w-full pointer-events-none transition-all duration-1000 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}
        style={{ maxWidth: "var(--max-w-content)", margin: "0 auto", paddingLeft: "var(--space-8)", paddingRight: "var(--space-8)" }}
      >
        <div className="flex flex-col lg:flex-row items-start lg:items-center">
          {/* Left column: text content */}
          <div className="flex flex-col items-start lg:w-[50%] pointer-events-auto pt-20 lg:pt-0">
            <div
              className="flex items-center border"
              style={{
                gap: "var(--space-2)", padding: "6px 16px", borderRadius: "var(--radius-full)",
                marginBottom: "var(--space-6)", background: "var(--surface-1)", borderColor: "rgba(168,85,247,0.3)",
              }}
            >
            ...
            <h1 className="font-black leading-none tracking-tighter" style={{ fontSize: "clamp(48px, 7vw, 80px)", marginBottom: "var(--space-3)" }}>
              ...
              <span className="block" style={{ fontSize: "clamp(38px, 5.5vw, 64px)", marginTop: "var(--space-2)", WebkitTextStroke: "1px rgba(168,85,247,0.5)", color: "transparent", letterSpacing: "-0.04em" }}>
                {siteContent.hero.lastName}
              </span>
            </h1>

            <div className="flex items-center" style={{ gap: "var(--space-3)", marginTop: "var(--space-3)", marginBottom: "var(--space-5)" }}>
            ...
            <p className="text-muted-foreground leading-relaxed" style={{ fontSize: "var(--text-md)", maxWidth: "520px", marginBottom: "var(--space-8)", lineHeight: "var(--leading-relaxed)" }}>
              {siteContent.hero.descriptionLines[0]}<br />{siteContent.hero.descriptionLines[1]}
            </p>
            ...
            <div className="flex items-center flex-wrap" style={{ gap: "var(--space-4)" }}>
              <a
                ...
                style={{ gap: "var(--space-3)", padding: "12px 28px", borderRadius: "var(--radius-full)", fontSize: "var(--text-sm)", background: "var(--btn-primary-bg)", color: "var(--primary-foreground)", boxShadow: "var(--btn-primary-shadow)" }}
              >
                {siteContent.hero.ctas.primary}
                <span className="w-5 h-5 rounded-full flex items-center justify-center transition-transform group-hover:translate-x-1" style={{ background: "rgba(255,255,255,0.2)", fontSize: "12px" }}>→</span>
              </a>
              <a
                href="#about"
                ...
                style={{ padding: "12px 28px", borderRadius: "var(--radius-full)", fontSize: "var(--text-sm)" }}
              >
            ...
            {/* Stats */}
            <div className="flex items-center flex-wrap border-t border-border" style={{ gap: "var(--space-8)", marginTop: "var(--space-10)", paddingTop: "var(--space-6)" }}>
              {siteContent.hero.stats.map((stat) => (
                <div key={stat.label} className="flex flex-col" style={{ gap: "var(--space-1)" }}>
                  <span className="font-bold" style={{ fontSize: "var(--text-2xl)", background: "var(--hero-stat-gradient)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
                    {stat.num}
                  </span>
                  <span className="text-muted-foreground tracking-widest uppercase" style={{ fontSize: "var(--text-xs)" }}>
                    {stat.label}
