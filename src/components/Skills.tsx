...
const Skills = () => {
  ...
  const capabilityCards =
    lang === "en"
      ? [
          { icon: <Sparkles size={14} />, title: "UI Systems", desc: "Figma/Sketch/PS/AI · components · multi-device consistency." },
          { icon: <Bot size={14} />, title: "AIGC Pipeline", desc: "MidJourney assets · visual library · production-ready efficiency." },
          { icon: <Code2 size={14} />, title: "Design to Build", desc: "Vibe coding · AI web rebuild · website production delivery." },
          { icon: <Wand2 size={14} />, title: "Creative Production", desc: "UI illustration/IP · campaign visuals · motion · AR/VR · video." },
        ]
      : [
          { icon: <Sparkles size={14} />, title: "UI 系统设计", desc: "Figma/Sketch/PS/AI · 组件库 · 多端一致性。" },
          { icon: <Bot size={14} />, title: "AIGC 生产链路", desc: "MidJourney 素材 · 视觉库 · 生产级提效。" },
          { icon: <Code2 size={14} />, title: "设计到构建", desc: "vibe coding · AI 还原网页 · AI 生产网站。" },
          { icon: <Wand2 size={14} />, title: "综合创意制作", desc: "UI 插画/IP · 运营物料 · 动效 · AR/VR · 视频。" },
        ];
      ...
      <div className="container-standard">
        ...
        <div className={`border transition-all duration-700 ${inView ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`} style={{ background: "var(--card)", borderColor: "var(--border)", borderRadius: "var(--radius-2xl)", padding: "var(--space-8)", transitionDelay: "0.5s", marginBottom: "var(--space-12)" }}>
          <h3 className="font-semibold text-foreground tracking-wide" style={{ fontSize: "var(--text-sm)", marginBottom: "var(--space-5)" }}>
            {lang === "en" ? "Capability Tags" : "能力标签"}
          </h3>
          <div className="flex flex-wrap" style={{ gap: "var(--space-3)" }}>
            {SOFT_SKILLS.map((skill) => (
              <span
                key={skill}
                className="tag-hover text-muted-foreground border border-border"
                style={{
                  padding: "6px 16px",
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between" style={{ gap: "8px", marginBottom: "var(--space-3)" }}>
              <div>
                ...
                <p className="text-muted-foreground leading-relaxed" style={{ fontSize: "var(--text-xs)" }}>
                  {lang === "en"
                    ? "Full-stack designer: strategy, UI systems, AIGC, and production delivery."
                    : "全能型设计师：策略、UI 系统、AIGC 与生产级交付。"}
                </p>
              </div>
              <div className="hidden md:inline-flex items-center border text-muted-foreground" style={{ gap: "6px", padding: "3px 8px", borderRadius: "var(--radius-full)", fontSize: "10px", borderColor: "rgba(168,85,247,0.35)", background: "rgba(168,85,247,0.08)" }}>
                ...
                {lang === "en" ? "All-round designer profile" : "全能设计师画像"}
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4" style={{ gap: "8px", marginBottom: "8px" }}>
                ...
                <div key={item.title} className="border" style={{ borderRadius: "12px", padding: "10px", borderColor: "rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.02)" }}>
                  <div className="flex items-start" style={{ gap: "8px" }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      <h4 className="font-semibold text-foreground" style={{ fontSize: "12px", marginBottom: "2px", lineHeight: 1.25 }}>
                        {item.title}
                      </h4>
                      <p className="text-muted-foreground leading-relaxed" style={{ fontSize: "11px", lineHeight: 1.4 }}>
                        {item.desc}
                      </p>
                    </div>
                  </div>
